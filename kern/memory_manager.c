...
void env_table_ws_print(struct Env *curenv);
 ...
 uint32 pd_is_table_used(struct Env *e, uint32 virtual_address);
 void pd_set_table_unused(struct Env *e, uint32 virtual_address);
 void pd_clear_page_dir_entry(struct Env *e, uint32 virtual_address);
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	int num = ROUNDUP(size, PAGE_SIZE)/PAGE_SIZE;
	for(int i = 0; i < num; i++) {
...
void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 virtAddr = virtual_address;
	...
	for(int i=0; i<numPages; i++, virtAddr+=PAGE_SIZE)
		pf_remove_env_page(e,virtAddr);

	//2. Free ONLY pages that are resident in the working set from the memory
	for(int i=0; i<e->page_WS_max_size; i++)
	{
		uint32 Address = env_page_ws_get_virtual_address(e,i);
		if(Address>= virtual_address && Address < virtual_address + size)
		{
			uint32* page_table_ptr = NULL;
			struct Frame_Info* frame_info_ptr = get_frame_info(e->env_page_directory, (void*)Address, &page_table_ptr);
			if(frame_info_ptr != NULL)
			{
			      unmap_frame(e->env_page_directory,(void*)Address);
	...
	for(int i=0; i<numPages; i++, virtAddr += PAGE_SIZE)
	{
		if(pt_get_page_permissions(e,virtAddr)&PERM_BUFFERED)
		{
			if(pt_get_page_permissions(e,virtAddr)&PERM_MODIFIED)
			{
				uint32* page_table_ptr = NULL;
				struct Frame_Info* frame_info_ptr = get_frame_info(e->env_page_directory, (void*)virtAddr, &page_table_ptr);
	...
	for(int i=0; i<numPages; i++, virtAddr+=PAGE_SIZE) {
		uint32* page_table_ptr = NULL;
		get_page_table(e->env_page_directory,(void*)virtAddr,&page_table_ptr);
		if(page_table_ptr != NULL)
		{
			...
			if(isPTEmpty == 1)
			{
				kfree((void*)page_table_ptr);
				pd_clear_page_dir_entry(e,virtAddr);
			}
		}
	}
	...
	//Refer to the project presentation and documentationfordetails
}
...
void moveMem(struct Env* e, uint32 src_virtual_address, uint32 dst_virtual_address, uint32 size)
{
	...
	//TODO: [PROJECT 2023 - MS2 - [4] Bonus1] moveMem() [Kernel Side]
...
/// Dealing with page and page table entry flags
 ...
 uint32 pd_is_table_used(struct Env* ptr_env, uint32 virtual_address)
{
	return ( (ptr_env->env_page_directory[PDX(virtual_address)] & PERM_USED) == PERM_USED ? 1 : 0);
}

 void pd_set_table_unused(struct Env* ptr_env, uint32 virtual_address)
{
...
extern int __pf_read_env_table(struct Env* ptr_env, uint32 virtual_address, uint32* tableKVirtualAddress);
 ...
 void pt_set_page_permissions(struct Env* ptr_env, uint32 virtual_address, uint32 permissions_to_set, uint32 permissions_to_clear)
{
	uint32 * ptr_pgdir = ptr_env->env_page_directory ;
	uint32* ptr_page_table;
 ...
 void pt_clear_page_table_entry(struct Env* ptr_env, uint32 virtual_address)
{
	uint32 * ptr_pgdir = ptr_env->env_page_directory ;
	uint32* ptr_page_table;
	//if(get_page_table(ptr_pgdir, (void *)virtual_address, &ptr_page_table) == TABLE_NOT_EXIST)
...
// [2] Add a frame info to the storage of frames at the given index
 void add_frame_to_storage(uint32* frames_storage, struct Frame_Info* ptr_frame_info, uint32 index)
{
