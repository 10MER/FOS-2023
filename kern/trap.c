...
void __page_fault_handler_with_buffering(struct Env * curenv, uint32 fault_va)
{
	...
	if (env_page_ws_get_size(curenv) < curenv->page_WS_max_size) {
		...
		if (page_permissions & PERM_BUFFERED) {
			...
			frame_info_ptr = get_frame_info(curenv->env_page_directory, (void*) fault_va, &page_table);
			frame_info_ptr->isBuffered = 0;

			bufferlist_remove_page((page_permissions & PERM_MODIFIED) ? &modified_frame_list : &free_frame_list, frame_info_ptr);
		} else {
			// 1.2
			int ret = allocate_frame(&frame_info_ptr);
			if (ret != E_NO_MEM) {
				page_permissions |= (PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
				ret = map_frame(curenv->env_page_directory, frame_info_ptr, (void*) fault_va, page_permissions);
				if (ret != E_NO_MEM) {
					ret = pf_read_env_page(curenv, (void*) fault_va);
					// if not successfully read then check if it's a stack page
	...
	// Replacement:
		...
		uint32 victimVirtAddr;
		uint32 victim_index;
		uint32 working_set_size;
		...
		if(isPageReplacmentAlgorithmModifiedCLOCK()){
			...
			int true = 1;
			while (true){
				working_set_size = curenv->page_WS_max_size;
				starting_index = curenv->page_last_WS_index;
				wsIndex = starting_index;
				while (working_set_size > 0){
					wsIndex = (wsIndex % curenv->page_WS_max_size);
					temp_VA = env_page_ws_get_virtual_address(curenv, wsIndex);
					uint32 page_permissions = pt_get_page_permissions(curenv, temp_VA);
					if((page_permissions & PERM_MODIFIED) == 0 && (page_permissions & PERM_USED) == 0){
						victimVirtAddr = temp_VA;
						victim_index = wsIndex;
						curenv ->page_last_WS_index = ((wsIndex + 1) % curenv->page_WS_max_size);
						true = 0;
						break;
					}
					working_set_size--;
				...
				while (working_set_size > 0){
					wsIndex = (wsIndex%curenv->page_WS_max_size);
					uint32 temp_VA = env_page_ws_get_virtual_address(curenv, wsIndex);
					uint32 page_permissions = pt_get_page_permissions(curenv, temp_VA);
					...
					pt_set_page_permissions(curenv, temp_VA, 0, PERM_USED);
					working_set_size--;
					wsIndex++;
				}
			}
		...
		struct Frame_Info *frame_info = get_frame_info(curenv->env_page_directory,(void*) victimVirtAddr, &page_table_ptr);
		frame_info->environment = curenv;
		frame_info->isBuffered = 1;
		frame_info->va = victimVirtAddr;

		pt_set_page_permissions(curenv, victimVirtAddr, PERM_BUFFERED, PERM_PRESENT);
		uint32 page_permissions = pt_get_page_permissions(curenv, victimVirtAddr);
		...
		if (page_permissions & PERM_MODIFIED){
			...
			if (LIST_SIZE(&modified_frame_list) == getModifiedBufferLength()){
				struct Frame_Info *modified_page_ptr;
				LIST_FOREACH(modified_page_ptr, &modified_frame_list){
					int ret = pf_update_env_page(modified_page_ptr->environment,(void*) modified_page_ptr->va, modified_page_ptr);
					pt_set_page_permissions(modified_page_ptr->environment,modified_page_ptr->va, 0, PERM_MODIFIED);
					bufferlist_remove_page(&modified_frame_list,modified_page_ptr);
					bufferList_add_page(&free_frame_list, modified_page_ptr);
				}
			}
		}
		else{
		...
		// Placement:
		uint32 new_page_permissions = pt_get_page_permissions(curenv, fault_va);
		if (new_page_permissions & PERM_BUFFERED) {
			pt_set_page_permissions(curenv, fault_va, PERM_PRESENT, PERM_BUFFERED);
			uint32 *page_table = NULL;
			...
			bufferlist_remove_page((new_page_permissions & PERM_MODIFIED) ? &modified_frame_list : &free_frame_list, frame_info_ptr);
		}
		else {
			// 1.2
			int ret = allocate_frame(&frame_info_ptr);
			if (ret != E_NO_MEM) {
				new_page_permissions |= (PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
				ret = map_frame(curenv->env_page_directory, frame_info_ptr, (void*) fault_va, page_permissions);
				if (ret != E_NO_MEM) {
					ret = pf_read_env_page(curenv, (void*) fault_va);
					// if not successfully read then check if it's a stack page
		...
		env_page_ws_set_entry(curenv, victim_index, ROUNDDOWN(fault_va, PAGE_SIZE));
	}
