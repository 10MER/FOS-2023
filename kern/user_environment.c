...
DECLARE_START_OF(tst_table_replacement_basics)
DECLARE_START_OF(tst_table_replacement_clock)
DECLARE_START_OF(tst_table_replacement_lru)
//DECLARE_START_OF(tst_malloc)
DECLARE_START_OF(tst_freeHeap_1)
DECLARE_START_OF(tst_freeHeap_2)
...
DECLARE_START_OF(tst_mod_1);
DECLARE_START_OF(tst_mod_2);
DECLARE_START_OF(tst_mod_3);
DECLARE_START_OF(tst_mod_4);
DECLARE_START_OF(tst_mod_5);
DECLARE_START_OF(tst_mod_6);
DECLARE_START_OF(tst_mod_7);

DECLARE_START_OF(tst_mod_r1);
DECLARE_START_OF(tst_mod_10);
DECLARE_START_OF(tst_mod_11);

//2018
DECLARE_START_OF(tst_page_replacement_free_rapid_proc_1);
DECLARE_START_OF(tst_page_replacement_free_rapid_proc_2);
DECLARE_START_OF(tst_page_replacement_free_scarce_mem_1);
DECLARE_START_OF(tst_page_replacement_free_scarce_mem_master_2);
DECLARE_START_OF(dummy_process);
DECLARE_START_OF(tst_page_replacement_free_scarce_mem_master_3);
DECLARE_START_OF(tst_page_replacement_free_scarce_mem_slave_3_1);
DECLARE_START_OF(tst_page_replacement_free_scarce_mem_slave_3_2);
...
struct UserProgramInfo userPrograms[] = {
		...
		{ "tpr3", "Tests page replacement after removing n pages from the current rapidly growing process running ALONE", PTR_START_OF(tst_page_replacement_free_rapid_proc_1)},
		{ "tpr4", "Tests page replacement after removing n pages from the current rapidly growing process running with other processes", PTR_START_OF(tst_page_replacement_free_rapid_proc_2)},
		{ "tpr5", "Tests page replacement after handling memory scarceness for only running environment", PTR_START_OF(tst_page_replacement_free_scarce_mem_1)},
		{ "tpr6", "Tests page replacement after handling memory scarceness for multiple running processes using round robin", PTR_START_OF(tst_page_replacement_free_scarce_mem_master_2)},
		{ "dummy_process", "[Slave program] contains nested loops with random bounds to consume time", PTR_START_OF(dummy_process)},
		{ "tpr7", "Tests page replacement after handling memory scarceness for multiple running processes using MLFQ", PTR_START_OF(tst_page_replacement_free_scarce_mem_master_3)},
		{ "scarceMem3Slave_1", "[Slave program 1] of Test tst_page_replacement_free_scarce_mem_master_3", PTR_START_OF(tst_page_replacement_free_scarce_mem_slave_3_1)},
		{ "scarceMem3Slave_2", "[Slave program 2] of Test tst_page_replacement_free_scarce_mem_master_3", PTR_START_OF(tst_page_replacement_free_scarce_mem_slave_3_2)},
		...
		//[4] MODIFICATIONS
		//		{ "tmod1", "", PTR_START_OF(tst_mod_1)},
		//		{ "tmod2", "", PTR_START_OF(tst_mod_2)},
		//		{ "tmod3", "", PTR_START_OF(tst_mod_3)},
		//		{ "tmod4", "", PTR_START_OF(tst_mod_4)},
		//		{ "tmod5", "", PTR_START_OF(tst_mod_5)},
		//
		//		{ "tmod6", "", PTR_START_OF(tst_mod_6)},
		//		{ "tmod7", "", PTR_START_OF(tst_mod_7)},
		//
		//		{ "tmodr1", "", PTR_START_OF(tst_mod_r1)},
		//
		//		{ "tmod10", "", PTR_START_OF(tst_mod_10)},
		//		{ "tmod11", "", PTR_START_OF(tst_mod_11)},
		/*TESTING 2015*/
		//[1] READY MADE TESTS
		//		{ "tpp", "Tests the Page placement", PTR_START_OF(tst_placement)},
		//		{ "tia", "tests handling of invalid memory access", PTR_START_OF(tst_invalid_access)},
		//		{ "tpr1", "Tests page replacement (allocation of Memory and PageFile)", PTR_START_OF(tst_page_replacement_alloc)},
		//		{ "tpr2", "Tests page replacement (clock algorithm)", PTR_START_OF(tst_page_replacement_clock)},
		//		{ "tpr3", "Tests page replacement (LRU algorithm)", PTR_START_OF(tst_page_replacement_lru)},
		//		{ "tpr4", "tests page replacement (handling new stack and modified pages)", PTR_START_OF(tst_page_replacement_stack)},
		//		{ "tm1", "tests malloc (1): start address & allocated frames", PTR_START_OF(tst_malloc_1)},
		//		{ "tm2", "tests malloc (2): writing & reading values in allocated spaces", PTR_START_OF(tst_malloc_2)},
		//		{ "tm3", "tests malloc (3): check memory allocation and WS after accessing", PTR_START_OF(tst_malloc_3)},
		...
		//		{ "ttr1", "tests page table replacement (basic operations)", PTR_START_OF(tst_table_replacement_basics)},
		//		{ "ttr2", "tests page table replacement (clock algorithm)", PTR_START_OF(tst_table_replacement_clock)},
		//		{ "ttr3", "tests page table replacement (LRU algorithm)", PTR_START_OF(tst_table_replacement_lru)},
		//		//{ "tm", "tests malloc function", PTR_START_OF(tst_malloc)},
		//		{ "tfh1", "tests freeHeap function (basic operations)", PTR_START_OF(tst_freeHeap_1)},
		//		{ "tfh2", "tests freeHeap function (placement with scattered empty entries in WS's)", PTR_START_OF(tst_freeHeap_2)},
		...
		//		{ "sem2Slave", "[Slave program] of tst_semaphore_2master", PTR_START_OF(tst_semaphore_2slave)},
		//		{ "tshr1", "Tests the shared variables [create]", PTR_START_OF(tst_sharing_1)},
		//		//{ "shr2Slave", "[Slave program] of tst_sharing_1master", PTR_START_OF(tst_sharing_2slave)},
		//		{ "tshr3", "Tests the shared variables [Special cases of create]", PTR_START_OF(tst_sharing_3)},
		//		{ "tshr4", "Tests the free of shared variables after createSharedObject only", PTR_START_OF(tst_sharing_4)},
		//		{ "tshr5", "Tests the free of shared variables after both createSharedObject and getSharedObject", PTR_START_OF(tst_sharing_5_master)},
		//		{ "tshr5slave", "Slave program to be used with tshr5", PTR_START_OF(tst_sharing_5_slave)},
		...
		//		{ "tr1", "tests realloc (1): normal cases", PTR_START_OF(tst_realloc_1)},
		//		{ "tr2", "tests realloc (2): special cases", PTR_START_OF(tst_realloc_2)},
		//		{ "tfr1", "tests freeRAM (1): run in specific scenario", PTR_START_OF(tst_freeRAM_1)},
		//		{ "tfr2", "tests freeRAM (2): run directly", PTR_START_OF(tst_freeRAM_2)},
...
void __env_free_with_buffering(struct Env *e)
{
    ...
    // Un-buffer pages fel modified list
    LIST_FOREACH(frame_info_ptr, &modified_frame_list)
    {
        if (frame_info_ptr->environment == e)
        {
            bufferlist_remove_page(&modified_frame_list, frame_info_ptr);
            frame_info_ptr->isBuffered = 0;
            frame_info_ptr->environment = NULL;
            free_frame(frame_info_ptr);
        }
    }

    // Un-buffer pages fel free list
    LIST_FOREACH(frame_info_ptr, &free_frame_list)
    {
        if (frame_info_ptr->environment == e)
        {
            bufferlist_remove_page(&free_frame_list, frame_info_ptr);
            frame_info_ptr->isBuffered = 0;
            frame_info_ptr->environment = NULL;
            free_frame(frame_info_ptr);
        }
	...
	i = 0;
	int maxUser = PDX(USER_TOP);
	for (; i < maxUser; i++) {

		uint32 physAddr = (e->env_page_directory[i] >> 12);
		physAddr *= PAGE_SIZE;

		uint32 vir = kheap_virtual_address(physAddr);
